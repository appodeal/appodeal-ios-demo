platform :ios, '15.0'
workspace 'AppodealDemo.xcworkspace'

source 'https://github.com/appodeal/CocoaPods.git'
source "https://github.com/bidon-io/CocoaPods-Specs.git"
source 'https://cdn.cocoapods.org/'

install! 'cocoapods',
  :deterministic_uuids => false,
  :warn_for_multiple_pod_sources => false

use_frameworks!

def appodeal
   pod 'Appodeal', '4.4.0'
   # Bidon
   pod 'BidonAdapterAmazon', '5.5.0.0'
   pod 'BidonAdapterAppLovin', '13.6.3.0'
   pod 'BidonAdapterBidMachine', '3.7.1.1'
   pod 'BidonAdapterBigoAds', '5.2.1.0'
   pod 'BidonAdapterDTExchange', '8.4.7.0'
   pod 'BidonAdapterInMobi', '11.3.0.0'
   pod 'BidonAdapterIronSource', '9.6.0.0.0'
   pod 'BidonAdapterMetaAudienceNetwork', '6.21.1.1'
   pod 'BidonAdapterMobileFuse', '1.11.0.0'
   pod 'BidonAdapterMoloco', '4.7.0.0'
   pod 'BidonAdapterMyTarget', '5.43.0.0'
   pod 'BidonAdapterStartIo', '4.11.0.0'
   pod 'BidonAdapterTaurusX', '1.9.2.0'
   pod 'BidonAdapterUnityAds', '4.20.0.0'
   pod 'BidonAdapterVungle', '7.7.3.0'
   pod 'BidonAdapterZmaticoo', '2.2.0.0'
   # AppLovin MAX
   pod 'AppLovinMediationAmazonAdMarketplaceAdapter', '5.5.0.0'
   pod 'AppLovinMediationBidMachineAdapter', '3.7.1.0.0'
   pod 'AppLovinMediationBigoAdsAdapter', '5.2.1.0'
   pod 'AppLovinMediationByteDanceAdapter', '7.9.1.1.0'
   pod 'AppLovinMediationChartboostAdapter', '9.13.0.0'
   pod 'AppLovinMediationFacebookAdapter', '6.21.1.0'
   pod 'AppLovinMediationFyberAdapter', '8.4.7.0'
   pod 'AppLovinMediationGoogleAdManagerAdapter', '13.5.0.0'
   pod 'AppLovinMediationGoogleAdapter', '13.5.0.0'
   pod 'AppLovinMediationInMobiAdapter', '11.3.0.1'
   pod 'AppLovinMediationIronSourceAdapter', '9.6.0.0.1'
   pod 'AppLovinMediationMintegralAdapter', '8.1.6.0.0'
   pod 'AppLovinMediationMobileFuseAdapter', '1.11.0.0'
   pod 'AppLovinMediationMolocoAdapter', '4.7.0.0'
   pod 'AppLovinMediationOguryPresageAdapter', '5.2.3.0'
   pod 'AppLovinMediationPubMaticAdapter', '5.1.1.0'
   pod 'AppLovinMediationSmaatoAdapter', '23.2.1.1'
   pod 'AppLovinMediationUnityAdsAdapter', '4.20.0.0'
   pod 'AppLovinMediationVerveAdapter', '3.8.1.0'
   pod 'AppLovinMediationVungleAdapter', '7.7.3.0'
   pod 'AppLovinMediationYandexAdapter', '8.3.0.0'
   pod 'TaurusxAdsSDK/AppLovinAdapter', '1.9.2'
   pod 'zMaticooMAXAdapter', '2.2.0'
   # Level Play
   pod 'IronSourceAdMobAdapter', '5.10.0.0'
   pod 'IronSourceAppLovinAdapter', '5.7.0.0'
   pod 'IronSourceBidMachineAdapter', '5.7.0.0'
   pod 'IronSourceBigoAdapter', '5.9.0.0'
   pod 'IronSourceFacebookAdapter', '5.3.0.0'
   pod 'IronSourceFyberAdapter', '5.8.0.0'
   pod 'IronSourceInMobiAdapter', '5.7.0.0'
   pod 'IronSourceMintegralAdapter', '5.19.0.0'
   pod 'IronSourceMobileFuseAdapter', '5.3.0.0'
   pod 'IronSourceMolocoAdapter', '5.13.0.0'
   pod 'IronSourceMyTargetAdapter', '5.9.0.0'
   pod 'IronSourceOguryAdapter', '5.6.0.0'
   pod 'IronSourcePangleAdapter', '5.19.0.0'
   pod 'IronSourceSmaatoAdapter', '5.5.0.0'
   pod 'IronSourceUnityAdsAdapter', '5.11.0.0'
   pod 'IronSourceVerveAdapter', '5.5.0.0'
   pod 'IronSourceVungleAdapter', '5.9.0.0'
   pod 'IronSourceYandexAdapter', '5.12.0.0'
   # Appodeal
   pod 'AppodealAdjustAdapter', '5.7.0.1'
   pod 'AppodealAmazonAdapter', '5.5.0.0'
   pod 'AppodealAppLovinAdapter', '13.6.3.0'
   pod 'AppodealAppLovinMAXAdapter', '13.6.3.0'
   pod 'AppodealAppsFlyerAdapter', '6.18.0.2'
   pod 'AppodealBidMachineAdapter', '3.7.1.0'
   pod 'AppodealBidonAdapter', '0.15.0.0'
   pod 'AppodealBigoAdsAdapter', '5.2.1.0'
   pod 'AppodealDTExchangeAdapter', '8.4.7.0'
   pod 'AppodealFacebookAdapter', '18.0.3.0'
   pod 'AppodealFirebaseAdapter', '12.15.0.1'
   pod 'AppodealGoogleAdMobAdapter', '13.5.0.0'
   pod 'AppodealIABAdapter', '4.4.0.0'
   pod 'AppodealInMobiAdapter', '11.3.0.0'
   pod 'AppodealIronSourceAdapter', '9.6.0.0.0'
   pod 'AppodealLevelPlayAdapter', '9.6.0.0.0'
   pod 'AppodealMetaAudienceNetworkAdapter', '6.21.1.0'
   pod 'AppodealMintegralAdapter', '8.1.6.0'
   pod 'AppodealMyTargetAdapter', '5.43.0.0'
   pod 'AppodealSentryAdapter', '8.57.2.1'
   pod 'AppodealUnityAdapter', '4.20.0.0'
   pod 'AppodealVungleAdapter', '7.7.3.0'
   pod 'AppodealYandexAdapter', '8.3.0.1'
end


target 'AppodealSwiftDemo' do
  project 'AppodealSwiftDemo/AppodealSwiftDemo.xcodeproj'

  appodeal
end

target 'AppodealObjectiveCDemo' do
  project 'AppodealObjectiveCDemo/AppodealObjectiveCDemo.xcodeproj'
  appodeal
end

target 'AppodealSwiftUIDemo' do
  project 'AppodealSwiftUIDemo/AppodealSwiftUIDemo.xcodeproj'
  appodeal
end

post_install do |installer|

  installer.pods_project.targets.each do |target|
    target.build_configurations.each do |config|
      # Patch min deploument target
      config.build_settings["IPHONEOS_DEPLOYMENT_TARGET"] = "15.0"
    end
  end
end
